...
1. Deleting the admin/auth/contenttypes records from django_migrations.
2. Running migrate which will reapply them in the correct dependency order.
"""
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            ...
            if admin_exists and not vendors_exists:
                ...
                # Remove admin/auth/contenttypes migrations so they re-run after vendors
                cursor.execute(
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'✅ Removed {deleted_count} conflicting records. '
                        f'migrate will now reapply cleanly.'
                    )
                )
            else:
